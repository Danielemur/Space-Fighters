...
void starship_fire(starship_t *starship, stage_t *stage)
{
  if (starship->charge > FIRE_DRAIN) {
    ...
    projectile_init(projectile, projectile_id, damage, (movement_t){entity->movement.position, vec2_scale(entity->movement.orientation, PROJECTILE_VELOCITY), {0, 0}});
...
void starship_update(entity_t *entity, stage_t *stage)
{
  ...
  starship->charge++;
  entity_living_update((entity_t*)starship, stage);
}
