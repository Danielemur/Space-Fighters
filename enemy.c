...
void enemy_update(entity_t *entity,stage_t *stage)
{
  ...
  if(best_distance<70){
    enemy->movement.velocity=vec2_scale(vec2_unitize(vec2_sub(closest_entity->movement.position,enemy->movement.position)),0.1);
    starship_fire(enemy);
  }else if(current_speed <ENEMY_SPEED){
    ...
    enemy->movement.acceleration=vec2_scale(vec2_negate(enemy->movement.orientation),vec2_mag(enemy->movement.acceleration));
  }    
